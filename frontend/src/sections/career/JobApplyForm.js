  ...
  const onSubmit = async (data) => {
    try {
      if (applyJob) {
        ...
        enqueueSnackbar('Application is not available.', { variant: 'error' });
      }
    } catch (error) {
      console.error(error);
      reset();
  ...
  return (
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Stack spacing={2.5}>
        ...
        {country === 'United States' && <Stack sx={{ justifyContent: 'flex-end' }} direction={{ xs: 'column', sm: 'row' }} spacing={2}>
          <RHFTextField name='ssn' label='SSN*' sx={{ width: '30%' }}/>
        </Stack>}
